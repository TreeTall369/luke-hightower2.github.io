...
function add(animals, animal){
    ...
    for (var i = 0; i < animals.length; i++) {
        return animals[i].name !== animal.name
    }
    }

    var booArray = []
    booArray.push(uniqueName())  

    animal.name !== undefined && animal.name.length > 0 ? booArray.push(true) : booArray.push(false); 
    animal.species !== undefined && animal.species.length > 0 ? booArray.push(true) :  booArray.push(false);  
    
    var resolve = booArray.every(val => val === true);
    if (resolve){animals.push(animal)}
    return animals
